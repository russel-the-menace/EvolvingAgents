...
import { Compass, MessageSquare, PanelLeftClose, PanelLeftOpen, Plus, SendHorizontal, Settings2 } from 'lucide-react';
...
export function App() {
  ...
  async function send() {
    ...
    try {
      const response = await fetch('/api/chat', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ messages: next.messages.map((message) => ({ role: message.role, content: message.text })) }) });
