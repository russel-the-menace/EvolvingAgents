...
type Message = { id: string; role: 'user' | 'assistant'; content: string; attachment?: ImageAttachment; product?: 'spot' | 'margin' | 'futures'; draft?: Draft; order?: Record<string, unknown> };
...
function readImageAttachment(file: File) {
  if (!['image/png', 'image/jpeg', 'image/webp'].includes(file.type)) return Promise.reject(new Error('只支持 PNG、JPEG 或 WebP 图片'));
  if (file.size > 4_000_000) return Promise.reject(new Error('图片不能超过 4 MB'));
  return new Promise<ImageAttachment>((resolve, reject) => {
    const reader = new FileReader();
    reader.onload = () => resolve({ dataUrl: String(reader.result), name: file.name || 'clipboard-image', type: file.type });
...
function TimeShareChart({ candles, selectedIndex, onSelect, onLoadOlder, onVisibleChange, last }: { candles: CoinMCandle[]; selectedIndex: number | null; onSelect: (index: number) => void; onLoadOlder: () => void; onVisibleChange: (candles: CoinMCandle[]) => void; last: number }) {
  ...
  const average = closes.map((_, index) => closes.slice(Math.max(0, index - 59), index + 1).reduce((sum, value) => sum + value, 0) / Math.min(index + 1, 60));
  ...
  const linePath = pathFor(closes); const areaPath = `${linePath} L ${pad + (visibleCandles.length - 1) * step + step / 2} ${height - pad} L ${pad + step / 2} ${height - pad} Z`;
  ...
  const applyZoomScale = (scale: number, centerFraction: number) => {
    const currentCount = visibleCountRef.current; const nextCount = Math.min(MAX_TIME_SHARE_POINTS, Math.max(MIN_TIME_SHARE_POINTS, Math.round(pinchStartCount.current / Math.max(.1, scale))));
    if (nextCount === currentCount) return;
    visibleCountRef.current = nextCount; setVisibleCount(nextCount); setHistoryOffset((current) => zoomWindowOffset(candles.length, current, currentCount, nextCount, centerFraction));
  };
  ...
  useEffect(() => {
    ...
    const onGestureStart = (event: Event) => { const gesture = event as Event & { scale?: number }; gestureStartScale = gesture.scale || 1; pinchStartCount.current = visibleCountRef.current; pinchStartDistance.current = 1; pinchAnchor.current = .5; event.preventDefault(); };
    const onGestureChange = (event: Event) => { const gesture = event as Event & { scale?: number }; applyZoomScale((gesture.scale || 1) / gestureStartScale, pinchAnchor.current); event.preventDefault(); };
    const onGestureEnd = () => { pinchStartDistance.current = null; window.localStorage.setItem(TIME_SHARE_ZOOM_KEY, String(visibleCountRef.current)); };
    svg.addEventListener('gesturestart', onGestureStart, { passive: false }); svg.addEventListener('gesturechange', onGestureChange, { passive: false }); svg.addEventListener('gestureend', onGestureEnd);
    return () => { svg.removeEventListener('gesturestart', onGestureStart); svg.removeEventListener('gesturechange', onGestureChange); svg.removeEventListener('gestureend', onGestureEnd); };
  }, [candles.length]);
...
function CoinMWorkspace({ onMarketContext }: { onMarketContext: (context: MarketContext) => void }) {
  ...
  useEffect(() => {
    ...
    const secondTimer = interval === '1s' && marketType === 'usdm' ? window.setInterval(() => { void api<CoinMMarket>(`/usdm-1s?symbol=${marketSymbol}`).then((result) => { if (active) setSecondRows(result.klines.map((row) => row)); }).catch(() => {}); }, 1_000) : null;
    let fallbackStarted = false;
    let directSockets: WebSocket[] = [];
    const connectDirect = () => {
      if (fallbackStarted || !active) return;
      fallbackStarted = true;
      const stream = interval === '1s' ? null : new WebSocket(`${streamHost}/${streamSymbol}@kline_${sourceInterval}`);
      if (stream) stream.onmessage = (event) => { let payload; try { payload = JSON.parse(event.data); } catch { return; } const item = payload.k; if (!active || !item) return; const next = [item.t, item.o, item.h, item.l, item.c, item.v, item.T, item.q]; setMarket((current) => current ? { ...current, klines: mergeKlineRows(current.klines, [next]) } : current); };
      const tradeStream = new WebSocket(`${streamHost}/${streamSymbol}@aggTrade`);
      tradeStream.onmessage = (event) => { let payload; try { payload = JSON.parse(event.data); } catch { return; } const price = Number(payload?.p); const quantity = Number(payload?.q); if (active && Number.isFinite(price)) { pendingPrice.current = price; if (interval === '1s' && Number.isFinite(quantity)) pendingSecondRows.current = mergeTradeIntoSecondRows(pendingSecondRows.current, Number(payload.T || payload.E || Date.now()), price, quantity); } };
      ...
      directSockets = [stream, tradeStream, depthStream].filter((socket): socket is WebSocket => Boolean(socket));
      for (const socket of directSockets) socket.onerror = () => { if (active) setError('实时行情暂时中断'); };
    };
    const serverStream = marketType === 'coinm' ? new EventSource(`/api/coinm-stream?interval=${interval === 'time' ? '1m' : interval}`) : null;
    if (serverStream) {
    ...
    return () => { active = false; window.clearInterval(timer); if (secondTimer !== null) window.clearInterval(secondTimer); window.clearInterval(bookTimer); window.clearInterval(relayTimer); serverStream?.close(); directSockets.forEach((socket) => socket.close()); };
  }, [interval, marketType]);
  ...
  const high = candles.length ? Math.max(...candles.map((item) => item.high), livePrice || -Infinity) : 1;
  ...
  const xAxisIndices = candles.length ? [...new Set([0, Math.floor((candles.length - 1) / 3), Math.floor((candles.length - 1) * 2 / 3), candles.length - 1])] : [];
  const axisTime = (time: number) => new Date(time).toLocaleString('zh-CN', interval === '1s' ? { hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false } : ['1d', '1w', '1M'].includes(interval) ? { month: '2-digit', day: '2-digit' } : { month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', hour12: false });
  ...
  return <div className={`coinm-page${interval === 'time' ? ' time-mode' : ''}`}>
    ...
    <div className="coinm-intervals coinm-full-intervals">{[['time', '分时'], ['1s', '1秒'], ['1m', '1分'], ['3m', '3分'], ['5m', '5分'], ['15m', '15分'], ['30m', '30分'], ['1h', '1小时'], ['2h', '2小时'], ['4h', '4小时'], ['6h', '6小时'], ['8h', '8小时'], ['12h', '12小时'], ['1d', '1天'], ['1w', '1周'], ['1M', '1月']].map(([id, label]) => <button className={interval === id ? 'active' : ''} key={id} onClick={() => { setInterval(id); setSelectedIndex(null); }}>{label}</button>)}</div>
    ...
    <section className="coinm-chart"><div className="coinm-intervals">{[['1s', '1秒'], ['1m', '1分'], ['3m', '3分'], ['5m', '5分'], ['15m', '15分'], ['30m', '30分'], ['1h', '1小时']].map(([id, label]) => <button className={interval === id ? 'active' : ''} key={id} onClick={() => { setInterval(id); setSelectedIndex(null); }}>{label}</button>)}</div><div className="coinm-ma-legend"><span>MA(7): {ma7.at(-1)?.toLocaleString(undefined, { maximumFractionDigits: 1 }) || '-'}</span><span>MA(25): {ma25.at(-1)?.toLocaleString(undefined, { maximumFractionDigits: 1 }) || '-'}</span><span>MA(99): {ma99.at(-1)?.toLocaleString(undefined, { maximumFractionDigits: 1 }) || '-'}</span></div>{candles.length ? <div className="coinm-chart-canvas"><svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label="BTCUSD coin-m candlestick chart" onClick={selectCandle}><g className="chart-grid">{[0, 1, 2, 3, 4].map((line) => <line key={line} x1={pad} x2={width - pad} y1={pad + line * (height - pad * 2) / 4} y2={pad + line * (height - pad * 2) / 4} />)}{xAxisIndices.map((index) => <line className="kline-date-guide" key={`time-${index}`} x1={pad + index * step + step / 2} x2={pad + index * step + step / 2} y1={pad} y2={height - pad} />)}</g>{candles.map((item, index) => { const x = pad + index * step + step / 2; const rising = item.close >= item.open; return <g className={rising ? 'candle-up' : 'candle-down'} key={item.time}><line x1={x} x2={x} y1={y(item.high)} y2={y(item.low)} /><rect x={x - Math.max(1, step * .28)} y={Math.min(y(item.open), y(item.close))} width={Math.max(2, step * .56)} height={Math.max(1, Math.abs(y(item.open) - y(item.close)))} /></g>; })}<path className="ma ma7" d={maPath(ma7)} /><path className="ma ma25" d={maPath(ma25)} /><path className="ma ma99" d={maPath(ma99)} />{last > 0 && <line className="current-price-guide" x1={pad} x2={width - pad} y1={y(last)} y2={y(last)} />}{selected && <g className="chart-crosshair"><line x1={pad + selectedIndex! * step + step / 2} x2={pad + selectedIndex! * step + step / 2} y1={pad} y2={height - pad} /><line x1={pad} x2={width - pad} y1={y(selected.close)} y2={y(selected.close)} /><circle cx={pad + selectedIndex! * step + step / 2} cy={y(selected.close)} r="4" /></g>}</svg><div className="coinm-axis">{axisValues.map((value, index) => <span key={index} style={{ top: `${index * 25}%` }}>{value.toLocaleString(undefined, { maximumFractionDigits: 1 })}</span>)}</div>{last > 0 && <div className="current-price-overlay" style={{ top: `${y(last) / height * 100}%` }}>{bookPrice(last)}</div>}<div className="kline-x-axis">{xAxisIndices.map((index) => <span key={candles[index].time} style={{ left: `${(pad + index * step + step / 2) / width * 100}%` }}>{axisTime(candles[index].time)}</span>)}</div>{selected && <div className="coinm-tooltip"><b>{new Date(selected.time).toLocaleString('zh-CN', { month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', second: interval === '1s' ? '2-digit' : undefined })}</b><span>开 <em>{selected.open.toLocaleString()}</em></span><span>高 <em>{selected.high.toLocaleString()}</em></span><span>低 <em>{selected.low.toLocaleString()}</em></span><span>收 <em>{selected.close.toLocaleString()}</em></span><span>涨跌 <em className={selected.close >= selected.open ? 'positive' : 'negative'}>{(selected.close - selected.open).toFixed(1)}</em></span><span>涨跌幅 <em>{((selected.close - selected.open) / selected.open * 100).toFixed(2)}%</em></span><span>振幅 <em>{((selected.high - selected.low) / selected.open * 100).toFixed(2)}%</em></span><span>量 <em>{selected.volume.toLocaleString()}</em></span><span>额 <em>{selected.quoteVolume.toLocaleString()}</em></span></div>}</div> : <div className="chart-empty">{error || (interval === '1s' ? '正在聚合 1 秒 K 线…' : '正在读取 K 线…')}</div>}</section>
  </div>;
}
...
function AssetWorkspace({ onMarketContext }: { onMarketContext: (context: MarketContext) => void }) {
...
export function App() {
  ...
  function newChat() { setActiveSessionId(null); setMessages([]); setInput(''); setAttachment(null); setError(''); }
  function openSession(session: ChatSession) { setActiveSessionId(session.id); setMessages(session.messages); setInput(''); setAttachment(null); setError(''); }

  async function send() {
    const content = input.trim() || (attachment ? '请分析这张图片。' : '');
    if (!content || busy) return;
    const sentAttachment = attachment;
    const user: Message = { id: crypto.randomUUID(), role: 'user', content, ...(sentAttachment ? { attachment: sentAttachment } : {}) };
    const sessionId = activeSessionId || crypto.randomUUID();
    const title = content.length > 32 ? `${content.slice(0, 32)}...` : content;
    const baseMessages = [...messages, user];
    setActiveSessionId(sessionId);
    setMessages(baseMessages); setInput(''); setAttachment(null); setBusy(true); setError('');
    try {
      const history = messages.slice(-12).map(({ role, content }) => ({ role, content }));
      const result = await api<{ reply: string; product?: Message['product']; draft?: Draft }>('/chat', { method: 'POST', body: JSON.stringify({ message: content, model: modelId, reasoning_effort: reasoningEffort, history, marketContext: marketContext.current, ...(sentAttachment ? { image: sentAttachment.dataUrl } : {}) }) });
