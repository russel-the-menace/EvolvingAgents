...
function CoinMWorkspace() {
  ...
  const candles = (market?.klines || []).map((item) => ({ time: Number(item[0]), open: Number(item[1]), high: Number(item[2]), low: Number(item[3]), close: Number(item[4]) }));
  ...
  const maPath = (period: number) => candles.map((_, index) => index < period - 1 ? null : candles.slice(index - period + 1, index + 1).reduce((sum, item) => sum + item.close, 0) / period).map((value, index) => value === null ? '' : `${index === period - 1 ? 'M' : 'L'} ${pad + index * step + step / 2} ${y(value)}`).join(' ');
  ...
  return <div className="coinm-page">
    ...
    <div className="coinm-trade-grid"><section className="coinm-order"><div className="coinm-order-tabs"><button className="active">开仓</button><button>平仓</button></div><div className="coinm-order-options"><button>全仓</button><button>20x</button></div><button className="coinm-select">市价单</button><label>数量 <span>张</span><input inputMode="decimal" placeholder="0" /></label><div className="coinm-order-buttons"><button disabled>开多 · 看涨</button><button disabled>开空 · 看跌</button></div></section><section className="coinm-book"><header><span>价格 (USD)</span><span>数量 (张)</span></header>{market?.depth.asks.slice().reverse().slice(0, 5).map(([price, quantity]) => <div className="ask" key={price}><span>{Number(price).toLocaleString()}</span><span>{Number(quantity).toLocaleString()}</span></div>)}<strong>{last.toLocaleString()}</strong>{market?.depth.bids.slice(0, 5).map(([price, quantity]) => <div className="bid" key={price}><span>{Number(price).toLocaleString()}</span><span>{Number(quantity).toLocaleString()}</span></div>)}</section></div>
    <div className="coinm-position-tabs"><b>持有仓位 (0)</b><span>当前委托 (0)</span><span>交易机器人</span></div>
    <section className="coinm-chart"><div className="coinm-intervals">{[['1m', '分时'], ['3m', '3分'], ['5m', '5分'], ['15m', '15分'], ['30m', '30分'], ['1h', '1小时']].map(([id, label]) => <button className={interval === id ? 'active' : ''} key={id} onClick={() => setInterval(id)}>{label}</button>)}</div>{candles.length ? <svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label="BTCUSD coin-m candlestick chart"><g className="chart-grid">{[0, 1, 2, 3, 4].map((line) => <line key={line} x1={pad} x2={width - pad} y1={pad + line * (height - pad * 2) / 4} y2={pad + line * (height - pad * 2) / 4} />)}</g>{candles.map((item, index) => { const x = pad + index * step + step / 2; const rising = item.close >= item.open; return <g className={rising ? 'candle-up' : 'candle-down'} key={item.time}><line x1={x} x2={x} y1={y(item.high)} y2={y(item.low)} /><rect x={x - Math.max(1, step * .28)} y={Math.min(y(item.open), y(item.close))} width={Math.max(2, step * .56)} height={Math.max(1, Math.abs(y(item.open) - y(item.close)))} /></g>; })}<path className="ma ma7" d={maPath(7)} /><path className="ma ma25" d={maPath(25)} /><path className="ma ma99" d={maPath(99)} /></svg> : <div className="chart-empty">{error || '正在读取币本位 K 线…'}</div>}<div className="coinm-chart-legend"><span>MA(7)</span><span>MA(25)</span><span>MA(99)</span><b>{last ? last.toLocaleString() : '-'}</b></div></section>
  </div>;
}
