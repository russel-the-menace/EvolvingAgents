...
function AssetWorkspace() {
  ...
  const rows = assetTab === 'spot' ? (spotView === 'spot' ? spot.map((item) => ({ asset: item.asset, primary: Number(item.free) + Number(item.locked), secondary: `可用 ${amount(item.free)} · 冻结 ${amount(item.locked)}` })) : []) : assetTab === 'funding' ? funding.map((item) => ({ asset: item.asset, primary: Number(item.free) + Number(item.locked || 0), secondary: `可用 ${amount(item.free)} · 冻结 ${amount(item.locked)}` })) : assetTab === 'earn' ? earnRows : futuresAssets.map((item) => ({ asset: item.asset, primary: item.walletBalance, secondary: `可用 ${amount(item.availableBalance)} · 未实现盈亏 ${amount(item.unrealizedProfit)}` }));
  ...
  return <section className="asset-app">
    <div className="asset-content">
      {bottomTab === 'assets' ? <>
        ...
        <div className="asset-summary"><div><span>{assetTab === 'overview' ? (data?.wallets?.length ? '预估总资产' : '现货 USDT 余额') : `${tabs.find(([id]) => id === assetTab)?.[1]}资产`}</span><button title={hidden ? '显示余额' : '隐藏余额'} aria-label={hidden ? '显示余额' : '隐藏余额'} onClick={() => setHidden((value) => !value)}><Eye size={15} /></button></div><strong>{amount(totalValue)} <small>USDT</small></strong>{assetTab === 'overview' && <div className="asset-cny">≈ ¥{amount(cnyTotal)}</div>}<div className="asset-actions"><button disabled title="资金操作尚未启用">添加资金</button><button disabled title="资金操作尚未启用">转出</button><button disabled title="资金操作尚未启用">划转</button></div></div>
        {assetTab === 'overview' ? <div className="wallet-overview"><div className="overview-tabs" role="tablist"><button className={overviewTab === 'all' ? 'active' : ''} onClick={() => setOverviewTab('all')}>全部</button><button className={overviewTab === 'accounts' ? 'active' : ''} onClick={() => setOverviewTab('accounts')}>账户</button><button className="overview-refresh" title="刷新资产" aria-label="刷新资产" onClick={() => void load()}><RefreshCw className={loading ? 'spin' : ''} size={15} /></button></div>{overviewTab === 'all' ? <>{spot.filter((item) => Number(item.free) + Number(item.locked) > 0).map((item) => <div className="asset-row" key={item.asset}><div><span className="asset-symbol">{assetIcon(item.asset)}</span><span><strong>{item.asset}</strong><small>{assetName(item.asset)}</small></span></div><b>{amount(Number(item.free) + Number(item.locked))}</b></div>)}</> : accountRows.map(([label, value]) => <div className="account-row" key={label}><strong>{label}</strong><b>{amount(value)} USDT</b></div>)}</div> : <div className="asset-list"><div className="asset-list-heading"><strong>{tabs.find(([id]) => id === assetTab)?.[1]}资产</strong>{assetTab === 'earn' ? <div className="inline-tabs"><button className={earnView === 'assets' ? 'active' : ''} onClick={() => setEarnView('assets')}>按资产</button><button className={earnView === 'products' ? 'active' : ''} onClick={() => setEarnView('products')}>按产品</button></div> : <Search size={17} />}</div>{rows.length ? rows.map((item) => <div className="asset-row" key={item.asset}><div><span className="asset-symbol">{assetIcon(item.asset)}</span><span><strong>{item.asset}</strong><small>{item.secondary || assetName(item.asset)}</small></span></div><b>{amount(item.primary)}</b></div>) : <p className="asset-empty">{loading ? '正在读取 Binance 账户…' : data?.errors.find((item) => item.startsWith(assetTab)) || '该账户暂无非零资产'}</p>}</div>}
        {data?.errors.length ? <details className="asset-errors"><summary>部分账户不可用</summary>{data.errors.map((item) => <p key={item}>{item}</p>)}</details> : null}
      </> : <div className="asset-placeholder"><strong>{nav.find(([, , id]) => id === bottomTab)?.[1]}</strong><span>此导航将在后续视图中实现</span></div>}
